...
alarm_list = []
notify_list = []
# retrieving path/name of local resources
...
s.enter(0, 1, initiate)  # scheduling the initiate function before app.run()
...
def main_page():
    ...
    # rendering the template during each refresh
...
def hourly_briefings(due_time):
    ...
    if retrieved_count > 0:  # announcing if at least 1 briefing was retrieved
        announce_join(notify_list[-retrieved_count::],
                      notify_list[-retrieved_count::], temp_list, True)
    s.enter(3600, 1, hourly_briefings, (datetime.now().strftime("%B %d, %H:%M"),))


def assign_briefings(due_time):
    """Retrieving and assigning briefing values"""
...
def assign_notifications(event, news, weather):
    """Used to retrieve and assign notifications"""
